...
from main_exp_subroutine import p_zS_givenR_for_m
from dataset import get_dataset, show_some_data, pick_test_idx
...
column_names = ['r_id', 'm', 's_id', 'z_id', 'loss', 'label', 'fro0', 'fro1', 'fro2', 'fro3', 'spe0','spe2']
...
def prob_zSR_for_m(config, device, prob_file, train_file):
    ...
    test_set_size = config.z_times

    r_times = config.r_times

    test_idx = pick_test_idx(len(data_dist), test_size = test_set_size)
    ...
    for r_id in tqdm(range(r_times), desc='over random init'):
        ...
        p_zS_givenR_for_m(config, data_dist, r_id, params, train_sets_size, device, prob_file, train_file, test_idx)
